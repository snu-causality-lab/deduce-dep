...
def cached_pc_stable_experiment(working_dir, BNs, CITs, Ks, Alphas, dataset_sizes, num_sampling, reliability_criteria,
                                n_jobs):
    # data-specific parameters
    Parallel(n_jobs=n_jobs)(delayed(__run_per_data)(Alphas, BN, Ks, ci_tester_name, reliability_criteria, sample_id,
                                                    size_of_sampled_dataset, working_dir) for
                            (BN, ci_tester_name), size_of_sampled_dataset, sample_id in product(list(zip(BNs, CITs)), dataset_sizes, list(range(1, num_sampling + 1))))


def __run_per_data(Alphas, BN, Ks, ci_tester_name, reliability_criteria, sample_id, size_of_sampled_dataset,
                   working_dir):
