...
def cond_evaluation(path, all_number_Para, target_list, real_graph_path, is_discrete, rule, filenumber=10, alaph=0.01, reliability_criterion='classic', K=1):
    ...
    for m in range(filenumber):
        ...
        for i, target in enumerate(target_list):
            Oraclepc = HITON_PC_oracle(data, assoc, target, alaph, is_discrete, true_graph)
            OraclePC[i] = Oraclepc

        sepsets = None
        for i, target in enumerate(target_list):
            start_time = time.time()
            PC, sepsets, ci_number = HITON_PC(data, assoc, target, alaph, is_discrete, reliability_criterion, K)
            end_time = time.time()
...
def pc_evaluation(path, real_graph_path, is_discrete, filenumber, alpha, reliability_criterion='classic'):
    # pre_set variables are zero
    Accuracy = 0
    Precision = 0
    Recall = 0
    ...
    for m in range(filenumber):
        ...
        start_time = time.time()
        estim_adj_mat, sepsets, ci_number = pc(data, alpha, is_discrete, reliability_criterion)
        end_time = time.time()
        time_lapsed = end_time - start_time
...
def pc_stable_evaluation(path, real_graph_path, is_discrete, filenumber=10, alpha=0.01, reliability_criterion='classic', K=1):
    # pre_set variables are zero
    Accuracy = 0
    Precision = 0
    Recall = 0
    ...
    for m in range(filenumber):
        ...
        start_time = time.time()
        estim_adj_mat, sepsets, ci_number = pc_stable(data, alpha, is_discrete, reliability_criterion, K)
        end_time = time.time()
        time_lapsed = end_time - start_time
        accuracy, precision, recall, f1 = global_skeleton_metric_evaluation(oracle_adj_mat, estim_adj_mat)
